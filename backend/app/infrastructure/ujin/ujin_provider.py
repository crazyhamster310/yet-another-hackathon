...
class UjinProvider(IUjinProvider):
    ...
    async def get_complexes(self) -> Dict:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.base_url}/api/v1/complex/list/", params={"token": self.token}, timeout=self.timeout)
    ...
    async def get_buildings(self, complexes: list[int] | None) -> Dict:
        data = {}
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.base_url}/api/v1/buildings/get-list-crm/", params=params, timeout=self.timeout)
                response.raise_for_status()
    ...
    async def get_parking(self, complexes: list[int] | None, buildings: list[int] | None) -> Dict:
        data = {}
    ...
    async def get_storage(self, complexes: list[int] | None, buildings: list[int] | None) -> Dict:
        data = {}
        params = {"token": self.token}
        if complexes:
            for complex_id in complexes:
    ...
    async def get_news(self, complexes: list[int] | None, buildings: list[int] | None, type_: list[str] | None) -> Dict:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.base_url}/api/v1/news/list", params=params, timeout=self.timeout)
                response.raise_for_status()

...
function WidgetRenderer({ widget }: { widget: any }) {
    ...
    if (loading && !data) return <Loader2 className="animate-spin text-slate-300" size={40} />;
    ...
    switch (widget.widget_type) {
        ...
        case "news":
            const newsItems = Array.isArray(data) ? data : data?.news?.all?.data?.items || [];
            return (
                <div className="w-full space-y-4 overflow-hidden">
                    {newsItems.slice(0, 2).map((item: any, i: number) => (
                        <div key={i} className="border-l-4 border-cyan-500 pl-4">
                            <div className="font-bold text-xl text-slate-800 line-clamp-1">{item.title}</div>
                            <div className="text-slate-500 text-sm line-clamp-2 mt-1" dangerouslySetInnerHTML={{ __html: item.text }} />
                        </div>
                    ))}
                </div>
            );
            ...
            return (
                <div className="flex flex-col items-start">
                    <div className="text-7xl font-black text-slate-900 tracking-tighter">
                        {data?.unassigned_count ?? 0} <span className="text-slate-300 font-light">/</span> {data?.total_count ?? 0}
                    </div>
...
export default function DisplayPlayer({ slug }: { slug: string }) {
    ...
    const [emergency, setEmergency] = useState<{ active: boolean; text: string }>({ active: false, text: "" });
    ...
    useEffect(() => {
        fetch(`/api/v1/screens/${slug}`)
            ...
            .then(data => {
                setConfig(data);
                if (data.is_emergency) setEmergency({ active: true, text: data.emergency_text || "" });

                const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
                const ws = new WebSocket(`${protocol}//${window.location.host}/ws/${data.id}`);
                ws.onmessage = (e) => {
                    const msg = JSON.parse(e.data);
                    if (msg.type === "EMERGENCY_UPDATE") {
                        setEmergency({ active: msg.payload.is_emergency, text: msg.payload.text || "" });
                    }
                };
                return () => ws.close();
            });
    ...
    if (!config) return <div className="h-screen w-screen bg-[#f1f5f9] flex items-center justify-center text-slate-400 font-bold">ЗАГРУЗКА ДИСПЛЕЯ...</div>;
    ...
    return (
        <div className="flex flex-col h-screen w-screen overflow-hidden bg-[#f1f5f9] font-sans">
            ...
            {emergency.active && (
                ...
                >
                    <AlertTriangle className="text-white flex-shrink-0" size={isTwoLines ? "5vh" : "3.5vh"} />
                    <div
                        ref={textRef}
                        ...
                        return (
                            ...
                            >
                                {slot ? (
                                    ...
                                    <div className="m-auto text-slate-200 font-black tracking-widest uppercase text-xs">Блок {idx + 1} свободен</div>
                                )}
                            </div>
                        );
                    })}

...
export default function AdminPanel() {
  ...
  const handleAssign = async (
    ...
    try {
      ...
      const res = await fetch(
        `/api/v1/screens/${screens.find((s) => s.id === screenId).slug}`,
      );
      const updatedScreen = await res.json();

      setScreens((prev) =>
        prev.map((s) => (s.id === screenId ? updatedScreen : s)),
      );
  ...
  const grouped = screens.reduce((acc: any, screen) => {
    ...
    if (!acc[cId])
      acc[cId] = { id: cId, title: `Жилой комплекс ${cId}`, buildings: {} };
    if (!acc[cId].buildings[bId])
      acc[cId].buildings[bId] = {
        id: bId,
        title: `Корпус ${bId}`,
        screens: [],
      };
      ...
      <main className="ml-16 p-10 w-full max-w-[1440px]">
        <div className="mb-6 flex items-center px-10 text-[13px] font-medium text-slate-400">
          ...
          <div className="w-[25%] flex items-center gap-1">
            <Monitor size={14} /> Адрес
          </div>
          <div className="w-[15%] flex items-center gap-1">
            <ArrowUp size={14} /> Аптайм
          </div>
          <div className="w-[15%] flex items-center gap-1">
            <Folder size={14} /> ID объекта
          </div>
          <div className="ml-auto flex items-center gap-1">
            <Plus size={14} /> Загрузка
          </div>
        </div>
        ...
        <div className="space-y-4">
          {Object.values(grouped).map((complex: any) => (
            <div
              key={complex.id}
              className="rounded-[2.5rem] bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] overflow-hidden"
            >
              <div
                className="flex items-center px-10 py-7 cursor-pointer hover:bg-slate-50/50 transition-colors"
                onClick={() => toggle(`c-${complex.id}`)}
              >
                ...
                <div className="w-[25%] text-slate-500 text-sm">
                  ул. Уличная, д. 15, к. 1-2
                </div>
                <div className="w-[15%]">
                  <StatusBadge status="Проблемы" />
                </div>
                <div className="w-[15%] flex items-center gap-2 text-slate-400 font-mono text-sm">
                  jk{complex.id}{" "}
                  <Copy
                    size={14}
                ...
                <button className="ml-auto bg-[#f8fafc] border border-slate-100 px-6 py-2.5 rounded-2xl text-[13px] font-bold text-slate-700 hover:shadow-sm transition-all">
                  Загрузить
                </button>
              </div>
              ...
              {expanded[`c-${complex.id}`] && (
                <div className="bg-[#f8fafc]/50 px-6 pb-6 space-y-3">
                  {Object.values(complex.buildings).map((building: any) => (
                    <div
                      key={building.id}
                      className="rounded-[2rem] bg-white border border-slate-100 shadow-sm overflow-hidden"
                    >
                      <div
                        className="flex items-center px-10 py-6 cursor-pointer"
                        onClick={() => toggle(`b-${building.id}`)}
                      >
                        ...
                        <div className="w-[15%]">
                          <StatusBadge status="Работает" />
                        </div>
                        <div className="w-[15%] flex items-center gap-2 text-slate-400 font-mono text-xs">
                          bld{building.id} <Copy size={12} />
                        </div>
                        <button className="ml-auto bg-[#f8fafc] border border-slate-100 px-6 py-2 rounded-2xl text-[13px] font-bold">
                          Загрузить
                        </button>
                      </div>
                      ...
                      {expanded[`b-${building.id}`] && (
                        <div className="px-10 pb-6 space-y-3">
                          {building.screens.map((screen: any) => (
                            <div
                              key={screen.id}
                              className="rounded-3xl border border-slate-50 overflow-hidden"
                            >
                              <div
                                className="flex items-center px-10 py-5 bg-white cursor-pointer hover:bg-slate-50/50"
                                onClick={() => toggle(`s-${screen.id}`)}
                              >
                                ...
                                <div className="w-[25%] text-slate-500 text-sm italic">
                                  {screen.slug}
                                </div>
                                <div className="w-[15%]">
                                  <StatusBadge
                                ...
                                <div className="w-[15%] flex items-center gap-2 text-slate-400 font-mono text-xs uppercase">
                                  scr{screen.id.slice(0, 5)} <Copy size={12} />
                                </div>
                                <button className="ml-auto bg-[#f8fafc] border border-slate-100 px-6 py-2 rounded-2xl text-[13px] font-bold">
                                  Загрузить
                                </button>
                                ...
                                <div className="p-10 bg-slate-50/30 border-t border-slate-50">
                                  <div className="flex items-center gap-4 mb-8">
                                    ...
                                    <span className="bg-[#dcfce7] text-[#166534] px-3 py-1 rounded-xl text-[11px] font-black uppercase tracking-tight flex items-center gap-1">
                                      <span className="w-1.5 h-1.5 bg-[#166534] rounded-full animate-pulse"></span>{" "}
                                      В эфире
                                    </span>
                                  </div>
                                  ...
                                  <div className="space-y-3 mb-8">
                                    {[0, 1, 2, 3].map((idx) => {
                                      ...
                                      return (
                                        <div
                                          key={idx}
                                          className="flex items-center bg-white px-8 py-5 rounded-[1.5rem] shadow-sm"
                                        >
                                          ...
                                          <div className="flex gap-2 relative">
                                            {assigningSlot?.sId === screen.id &&
                                            assigningSlot?.idx === idx ? (
                                              <div className="absolute right-0 bottom-full mb-2 w-64 bg-white shadow-xl rounded-2xl border border-slate-200 z-50 p-2 space-y-1">
                                                ...
                                                <button
                                                  ...
                                                  className="w-full text-slate-400 text-[10px] font-bold py-1 border-t mt-1"
                                                >
                                                  Отмена
                                                </button>
                                              </div>
                                  ...
                                  <div className="flex items-center justify-between bg-white px-10 py-4 rounded-[2rem] border border-slate-100 shadow-inner">
                                    ...
                                    <div className="flex gap-2 pr-2">
                                      ...
                                      <button
                                        onClick={() =>
                                          handleSendEmergency(screen.id, true)
                                        }
                                        className={clsx(
